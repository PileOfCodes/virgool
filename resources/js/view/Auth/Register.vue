<template>
    <div class="flex flex-col md:flex-row">
        ...
        <div class="flex flex-col md:h-screen md:basis-1/3 md:pt-24">
            <div class="text-sky-600 font-bold p-5"> ایجاد حساب کاربری</div>
            <input v-model="form.email" :class="[emailError ? 'border-red-500 border-2' : '']" 
            class="mr-8 border p-2 w-96 xl:w-96 md:w-80 rounded-md" placeholder=" ایمیل" @blur="checkEmail">
            <span class="mr-8 text-red-500 text-sm py-2 font-bold" v-if="emailError && serverEmailError == ''">{{ emailError }}</span>
            <span class="mr-8 text-red-500 text-sm py-2 font-bold" v-if="serverEmailError">{{ serverEmailError }}</span>
            <input type="password" v-model="form.password"
            :class="[passwordError ? 'border-red-500 border-2' : '']" class="mr-8 border p-2 w-96 md:w-80 xl:w-96 mt-2 rounded-md" @blur="checkPassword" placeholder="رمز عبور">
            ...
            <div class="flex justify-end ml-8 lg:ml-8 md:ml-0">
                <button  @click="register" class="flex flex-row justify-center items-center text-white mt-4 bg-sky-600 p-2 h-10 w-48 rounded-xl">
            ...
            <div class="flex flex-col justify-center items-center mt-24 text-sm">
                ...
                <router-link class="pb-12 md:pb-4" :to="{ name: 'login'}">قبلا عضو شده اید ؟ رفتن به صفحه ورود</router-link>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        register() {
            ...
            .catch((error) => {
                this.serverEmailError = error.response.data.errors.email[0]
            })
